...
%HERE
The jet energy resolution dependence on the HCal cell size is less strong than that observed in the ECal cell size, but that is to be expected as, in general, the ECal cell size determines the start position of showering particles in the calorimeters.  If the start of a particle shower is well reconstructed then it becomes easier to associate any remaining energy deposits to it.  This can be seen when making a comparison between the ECal and HCal cell size studies as it is clear that ECal cell size is more crucial for detector performance than the HCal cell size is.
