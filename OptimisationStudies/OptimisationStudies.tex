...
The confusion contribution to the jet energy resolution is reduced by improving the intrinsic energy resolution of a detector as it leads to more accurate energy comparisons between clusters of calorimeter hits and charged particle tracks, which leads to fewer calorimetric energy deposits where the energy is either double counted or not counted at all.  When the number of layers in the ECal is increased the intrinsic energy resolution of the detector improves, which has the knock on effect of reducing the confusion contribution to the ket energy resolution.  This trend can be seen in the jet energy resolution as a function of the number of layers in the ECal is shown in figure \ref{fig:ecalsinlayers} for the silicon option and figure \ref{fig:ecalscnlayers} scintillator option.  For both options the jet energy resolution was found to improve with increasing number of layers in the ECal, however, the magnitude of the change was strongly dependent upon the jet energy.  For low energy jets the jet energy resolution dependancy on the number of ECal layers is much stronger than observed for high energy jets.  The origin of this trend is the energy dependance of the stochastic term in the energy resolution for a sampling calorimeter, which is $\propto \frac{1}{\sqrt{E}}$.  For high jet energies the energy resolution in the ECal is very low, which means the changes to the stochastic term that occur when varying the number of layers in the ECal are too fine to be resolved in the jet energy resolution.  Whereas for low jet energies the stochastic term is bigger making it possible to resolve the changes to it when varying the number of ECal layers.  The decomposition of the jet energy resolution into the intrinsic energy resolution and confusion contributions for 45 and 250 GeV jets using both the silicon and scintillator ECal options are shown in figure \ref{fig:ecalnlayersbreak}.  As expected the twofold effect of a reduction in both the intrinsic energy resolution and the confusion contributions to the jet energy resolution is observed when increasing the number of layers in the ECal.  Furthermore, the changes to the jet energy resolution contributions when varying the number of layers in the ECal are similar indicating that the overall trend in jet energy resolution is being simultaneously driven by a combination of these effects.
...
\begin{itemize}
\item Cell size or cell size.  This is key to successful application of pattern recognition in the particle flow paradigm, but should not change intrinsic energy resolution.   
...
\begin{figure}
\centering
\subfloat[45 GeV Jets.]{\label{fig:hcalabsmaterial45break}\includegraphics[width=0.5\textwidth]{OptimisationStudies/Plots/JetEnergyResolutions/JER_vs_HCalAbsorberMaterial_91GeV_DiJet_Breakdown.pdf}}
\subfloat[250 GeV Jets.]{\label{fig:hcalabsmaterial250break}\includegraphics[width=0.5\textwidth]{OptimisationStudies/Plots/JetEnergyResolutions/JER_vs_HCalAbsorberMaterial_500GeV_DiJet_Breakdown.pdf}}
