void TidyECalMIPScale()
{
    gROOT->ProcessLine(".x Direction_Corrected_SimCalorimeterHit_Energy_Distribution_ECal_10_GeV_Muons.C");
    TH1F *pTH1F = ECalDirectionCorrectedSimCaloHit;
    ...
    pTH1F->GetXaxis()->SetRangeUser(0,0.0004);
    ...
    pTCanvas->Update();
    DrawVerticalLine(0.0001475);

    pTCanvas->SaveAs("MIPScaleDigitiserECal.pdf");
    pTCanvas->SaveAs("MIPScaleDigitiserECal.C");
