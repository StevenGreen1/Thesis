void TidyHCalBarrelDigi()
{
    gROOT->ProcessLine(".x Calorimeter_Hit_Energies_HCal_Barrel_Digitisation.C");
    TH1F *pTH1F = CaloHitEnergyHCalBarrel;

    TF1 *pTF1 = new TF1("GaussianFit","[0]*TMath::Exp(-0.5*[2]*TMath::Power(x-[1],2))",16,25);
    pTF1->SetLineColor(kRed);
    pTF1->SetLineWidth(2);
    pTF1->FixParameter(0,700);
    pTF1->SetParError(0,0);
    pTF1->SetParLimits(0,0,0);
    pTF1->SetParameter(1,20);
    pTF1->SetParError(1,0.0);
    pTF1->SetParLimits(1,0,0);
    pTF1->SetParameter(2,1.04604e-01);
    ...
    pTF1->Draw("same");
    pTLegend->Draw();
    pTCanvas->SaveAs("DigitisationHCalBarrelFit.pdf");
    pTCanvas->SaveAs("DigitisationHCalBarrelFit.C");
