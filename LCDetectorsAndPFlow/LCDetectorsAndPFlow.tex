...
\end{itemize}
\noindent These effects, collectively know as "confusion", degrade the energy resolution of a particle flow detector.  Therefore, it is crucial to make correct associations between charged particle tracks and their calorimetric energy deposits to minimise the effect of confusion.  These associations can only be successfully made if the calorimeters used have fine segmentation, such as those found at the linear collider experiment, so that it becomes possible to separate the energy deposits from nearby showering particles.  Even with this segmentation, making the association of charged particle tracks to calorimetric energy deposits is highly non-trivial.  At the linear collider experiment, these associations are made using sophisticated pattern recognition algorithms, provided by PandoraPFA \cite{Marshall:2015rfa}.  The fine segmentation of the linear collider calorimeters allows PandoraPFA to reconstruct the four-momenta of all particles \textcolor{entering the calorimeter} and to report the energy of all reconstructed particles using energy measurements from the optimal sub-detectors.  

\textcolor{blue}{There are two classifications of particle showers than can be produced when a particle interacts with a calorimeter; electromagnetic and hadronic...

Electromagnetic showers are produced when either photons, electrons or positrons interact with matter in the calorimeter.  A simple model of an electromagnetic shower works by assuming that above a critical energy, $E_{C}$, electrons and positrons only lose energy via bremsstrahlung and that photons only undergo pair production.  Below this energy ionisation and excitation of atomic nuclei dominates and no further particles are produced in the shower.  Above $E_{C}$, the average distance any particle in the electromagnetic shower travels before undergoing bremsstrahlung or pair production is approximately one radiation length ($X_{0}$).  Assuming that when the bremsstrahlung or pair production mechanisms take place the parent particle energy is split equally between the daughter products then, every $X_{0}$ the total number of particles doubles and the average particle energy halves.  In this case, the peak number of particles occurs after $t_{max}$ interaction lengths, where $t_{max} = \text{ln}(E_{Incident}/{E_{C}})/ln(2)$.  Therefore, for a 10~GeV incident photon, the peak number of particles occurs after $\sim10$ interaction lengths and at that point the particle shower contains $\sim1000$ particles.  These particles will be largely confined within a cylinder surrounding the shower core with a radius given by the Moli\`{e}re} radius for the material the photon showers within, which for tungsten is 0.93~cm or roughly 2 cell widths.  Therefore, the approximate occupancy of ECal cells for such a shower would be of order of 50-100 particles per cell.}
