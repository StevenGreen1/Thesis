...
The isolated lepton finder determines whether a PFO is an electron or muon by first checking that the PFO has a single charged particle track associated to it.  If that is the case, the calorimetric energy deposits of the PFO are examined to see if they are consistent with what is expected for an electron or muon.  If they are consistent with expectations, the properties of the charged particle track are examined to determine whether the track originates from the IP.  If the PFO is deemed to have originated from the IP, isolation checks \textcolor{blue}{that} examine the energy deposited in the calorimeters within a cone surrounding the PFO, are applied to determine whether the particle belongs to a jet.  If the PFO does not appear to belong to a jet then it is counted as an isolated lepton.  The fraction of events rejected by the lepton finder is summarised in table \ref{table:efficiencyleptonfinding}.   
...
\hline
\end{tabular}
\caption[The fraction of events rejected by the isolated lepton finding at $\sqrt{s}=1.4$~TeV for the {\nu}{\nu}qqqq and {\nu}lqqqq final states.]{The fraction of events rejected by the isolated lepton finding at $\sqrt{s}=1.4$~TeV for the {\nu}{\nu}qqqq and {\nu}lqqqq final states.}
...
\begin{figure}[h!]
\centering
\subfloat[]{\label{fig:ranking1}\includegraphics[width=0.5\textwidth]{PhysicsAnalysis/Plots/HighRankingVariables/1400GeV/AcolinearitySynBosons_1400GeV_Pt_gt100GeV_NIsoLep_eq0_Cuts_StackPlot.pdf}}
\subfloat[]{\label{fig:ranking2}\includegraphics[width=0.5\textwidth]{PhysicsAnalysis/Plots/HighRankingVariables/1400GeV/InvariantMass_HighestEnergySynBoson_1400GeV_Pt_gt100GeV_NIsoLep_eq0_Cuts_StackPlot.pdf}} \hfill
\subfloat[]{\label{fig:ranking3}\includegraphics[width=0.5\textwidth]{PhysicsAnalysis/Plots/HighRankingVariables/1400GeV/NPfos_HighestEnergyJet_1400GeV_Pt_gt100GeV_NIsoLep_eq0_Cuts_StackPlot.pdf}} 
\includegraphics[width=0.5\textwidth]{PhysicsAnalysis/Plots/PreSelection/1400GeV/Legend.pdf}
\caption[Distributions of the highest ranked variables used by the BDT: \protect\subref{fig:ranking1} the acolinearity of the candidate boson pair; \protect\subref{fig:ranking2} the invariant mass of the highest energy candidate boson; and \protect\subref{fig:ranking3} the number of PFOs in the highest energy jet.  All distributions includes effect of preselection and are normalised to an integrated luminosity of $\mathcal{L}_{int} = 1.5\text{ ab}^{-1}$.]{\textcolor{blue}{Distributions of the highest ranked variables used by the BDT: \protect\subref{fig:ranking1} the acolinearity of the candidate boson pair; \protect\subref{fig:ranking2} the invariant mass of the highest energy candidate boson; and \protect\subref{fig:ranking3} the number of PFOs in the highest energy jet.  All distributions includes effect of preselection and are normalised to an integrated luminosity of $\mathcal{L}_{int} = 1.5\text{ ab}^{-1}$.}}
