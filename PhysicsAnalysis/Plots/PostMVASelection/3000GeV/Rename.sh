...
sed -i -e 's/->GetXaxis()->SetLabelSize(0.035);/->GetXaxis()->SetLabelSize(0.05);/g' $1
sed -i -e 's/->GetYaxis()->SetLabelSize(0.035);/->GetYaxis()->SetLabelSize(0.05);/g' $1
sed -i -e 's/->GetXaxis()->SetTitleSize(0.035);/->GetXaxis()->SetTitleSize(0.05);/g' $1
sed -i -e 's/->GetYaxis()->SetTitleSize(0.035);/->GetYaxis()->SetTitleSize(0.05);/g' $1
sed -i -e "s/->GetXaxis()->SetTitle(.*);/->GetXaxis()->SetTitle(\"$2\");/g" $1
sed -i -e "s/->GetYaxis()->SetTitle(.*);/->GetYaxis()->SetTitle(\"Entries\");/g" $1
sed -i -e "s/0.7,0.7,1,1/0.75,0.1,0.95,0.95/g" $1
 
sed -i -e "s/->SetMaximum(.*);/->SetMaximum(2000000);/g" $1
sed -i -e "s/->SetMinimum(.*);/->SetMinimum(1);/g" $1
