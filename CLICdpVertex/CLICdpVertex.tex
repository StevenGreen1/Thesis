...
The mean ToT measured in the CLICpix as a function of the CCPDv3 output voltage pulse height is shown in figure \ref{fig:tot}.  The determination of the mean and error bars for the ToT measurement is identical to that described above for the rise time as a function of pulse height plot.  
...
Capacitive coupling of the sensor to a readout ASIC may lead to the unwanted effect of cross coupling.  Cross coupling is the unwanted transfer of signal from a given pixel on the sensor to neighbouring pixels in the readout ASIC and can be expected to occur if there is a non-zero capacitance, cross-capacitance, between the two.  This issue is of special interest in this analysis as any misalignments between the CCPDv3 and CLICpix will lead to an increase in the cross-capacitance between two pixels depending on the direction of the misalignment.  

Following the same setup as was used in section \ref{sec:resultstotpulseheight} for the ToT against pulse height analysis, but this time considering the ToT on the adjacent CLICpix pixel along the direction of the misalignment, it is possible to determine the effect of any cross coupling in these devices.  The mean ToT on the adjacent pixel is shown as a function of the pulse height for all devices where the CCPDv3 and CLICpix are aligned in figure \ref{fig:totcrosscoupling1} and for the misaligned samples in figure \ref{fig:totcrosscoupling2}.
...
\caption[The mean ToT measured on the adjacent pixel, along the direction of the offset, of the CLICpix ASIC as a function of CCPDv3 voltage pulse height.]{The mean ToT measured on the adjacent pixel, along the direction of the offset, of the CLICpix ASIC as a function of CCPDv3 voltage pulse height.}
\end{figure}

% HERE

The distribution of mean ToT on the adjacent CLICpix pixel against the pulse height for the centred samples show that there is no strong correlation between the ToT and pulse height.  This indicates that the cross-capacitance for the case of nominal alignment is sufficiently small that cross coupling should not be a problem.  
